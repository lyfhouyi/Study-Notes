...
void mainImage(out vec4 fragColor,in vec2 fragCoord)
{
    ...
    float time=progress;
    float theta=2.*pi*time;//自旋角度
    float disCenterAxis=abs(uv.x-.5);//当前点到中轴距离
    float pointUnwrappedX=.5+(uv.x-.5)/cos(theta);//对应的未折叠点_X
    bool outside=pointUnwrappedX>1.||pointUnwrappedX<0.;//对应的未折叠点是否在视口外
