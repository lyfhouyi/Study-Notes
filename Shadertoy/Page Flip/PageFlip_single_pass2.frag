...
void mainImage(out vec4 fragColor,in vec2 fragCoord)
{
    ...
    float vAxis=-3.*iResolution.x;//圆柱运动速度
    float process=fract(iTime/6.);//特效进度
    vec2 vDirection=normalize(vec2(-5.,1.));//圆柱运动方向（与圆柱轴线垂直）
    
    vec2 pointAxisBottom=vec2(iResolution.x+process*vAxis,0.);//中轴与屏幕下边界交点，将第一张图 (iResolution.x, 0.0) 点与圆柱初始中轴处固接
    ...
    if(pointUnwrapped.y>0.&&onCylinder&&pointUnwrapped.x>iResolution.x&&pointUnwrapped.x<1.02*iResolution.x){
        //左方阴影
        color.rgb=vec3(1.,0.,0.);
    }
    // disCurAxis>-0.2*iResolution.x
    //  behindAxis && !onCylinder &&
    if(!onCylinder &&behindAxis&& disCurAxis>(iResolution.x-0.8*pointUnwrapped.x)){
        //右方阴影
