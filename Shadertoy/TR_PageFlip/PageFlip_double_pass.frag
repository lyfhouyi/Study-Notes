...
void mainImage(out vec4 fragColor,in vec2 fragCoord)
{
    ...
    if(pointUnwrapped.y>0.&&onCylinder&&pointUnwrapped.x>.995*iResolution.x&&pointUnwrapped.x<1.005*iResolution.x){
        //左方抗锯齿
        fragColor=gaussianBlur(fragCoord);
    }else if(behindAxis&&abs(disCurAxis)>.995*r&&abs(disCurAxis)<1.005*r){
        //右方抗锯齿
        fragColor=gaussianBlur(fragCoord);
    }
    else{
